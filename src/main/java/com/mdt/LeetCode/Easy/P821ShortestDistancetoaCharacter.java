...
public class P821ShortestDistancetoaCharacter {
    ...
    public int[] shortestToChar(String s, char c) {
        ...
        var charOccurrences = new ArrayList<Integer>();
        ...
        for (var i = 0; i < chars.length; i++) {
            if (j != charOccurrences.size() && i > charOccurrences.get(j))
                j++;

            ans[i] = Math.min(j == charOccurrences.size() ? Integer.MAX_VALUE : Math.abs(i - charOccurrences.get(j)),
                    j - 1 < 0 ? Integer.MAX_VALUE : Math.abs(i - charOccurrences.get(j - 1)));
        }
